...
def extractConfidenceLabels():
    ...
    for index, row in MT_labels.iterrows():
        ...
        # data = data.append(pd.DataFrame([round(row[rate_type + '_rms'])], columns=['label'], index=[index]))
        # print(round(row[rate_type + '_rms']))

        if row[rate_type+ '_rms'] >= median[rate_type+ '_rms']:
    ...
    # # print(len(pers_keys))
    data.to_csv('../data/MT/' + rate_type + 'Label.csv')
