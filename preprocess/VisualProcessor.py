...
import sys
sys.path.append('/tsi/clusterhome/abarkar/CMU-MultimodalSDK')
...
# extract au presence related features
def au_presence(f):
    data = f[:,693:]
    data[np.isnan(data)] = 0
    # print(data.shape)
    presences=np.sum(data, axis=0)
    episodes=np.repeat(0, data.shape[1])
    for u in range(0, data.shape[0]-1):
        for j in range(0, data.shape[1]):
            if (data[u][j]!=data[u+1][j]):
                episodes[j] += 1
    episodes=episodes/2
    average = np.zeros(episodes.shape)
    for i in range(0,len(episodes)):
        if episodes[i] !=0:
            average[i]=presences[i]/episodes[i]
        else:
            average[i] = presences[i] / 1
    return list(presences) + list(episodes) + list(average)

# extract au intensity related features
def au_intensity(f):
    data = f[:,693-17:693]
    data[np.isnan(data)] = 0
    data_diff = np.vstack([np.zeros((1,data.shape[1])), np.diff(data, axis=0)])
    all = list(np.mean(data,axis=0))+ list(np.std(data,axis=0))+\
          list(np.max(data,axis=0)-np.min(data,axis=0)) + \
          list(np.mean(data_diff,axis=0))+list(np.std(data_diff,axis=0))+ \
          list(np.max(data_diff,axis=0) - np.min(data_diff,axis=0))
    return all

# extract facial expression related features
def VisualProcessor():
    keys = confidenceKeys()
    ...
    index = []
    for k in tqdm(keys):
        f = np.array(openface[str(k)]['features'])
        au_presen = au_presence(f)
        au_inten = au_intensity(f)
        visualFeature.append(au_presen + au_inten)
        index.append(k)
    scalar = StandardScaler()
    visualFeature = scalar.fit_transform(visualFeature)
    visualFeature = pd.DataFrame(visualFeature, columns=au_presence_n + au_intensity_n, index=index)
    visualFeature.to_csv('../data/visual.csv')
